    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    QObject::connect(ui->vsnBtnRecogObj, SIGNAL(pressed()), this, SLOT(vsnBtnRecogObjectPressed()));
...
void MainWindow::spgTxtSayReturnPressed(){
    this->commNode->publish_tts_query(this->ui->spgTxtSay->text().toStdString());
}

void MainWindow::sprTxtFakeRecogReturnPressed(){
}

void MainWindow::vsnBtnRecogObjectPressed(){
    std::string obj_id = this->ui->vsnTxtObjName->text().toStdString();
    if(obj_id == "")
	this->ui->vsnTxtObjName->setText("all");
    bool success = false;
    std::vector<std::string> results;
    if(obj_id == "" || obj_id == "any" || obj_id == "all")
	success = this->commNode->call_recog_objs(results);
    else if(this->commNode->call_recog_obj(obj_id))
	results.push_back(obj_id);
    if(results.size() == 0)
	results.push_back("None");
    std::string all_names = "Found: ";
    for(size_t i=0; i < results.size(); i++)
	all_names += results[i] + " ";
    this->ui->vsnLblFoundObj->setText(all_names.c_str());
}
