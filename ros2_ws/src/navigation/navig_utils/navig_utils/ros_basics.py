...
class RosBasicsNode(Node):
    ...
    def spin(self):
        while rclpy.ok():
            ...
            #
            # Primero revisamos si nos vamos a estrellar de frente
            if self.obstacle_front:
                self.get_logger().info("Obstacle in front")
                self.move(0, 0.78, 4.0)
            # Si el frente está libre, revisamos los lados
    ...
    def callback_scan(self, msg):
        ...
        # Revisamos un "cono" de 40 rayos para el frente (20 a la izquierda y 20 a la derecha del centro)
        self.obstacle_front = min(msg.ranges[n//2 - 20 : n//2 + 20]) < 1.0
        
        # Cono para la izquierda
        self.obstacle_left = min(msg.ranges[n//2 + 80 : n//2 + 120]) < 1.0
        
        # Cono para la derecha
        self.obstacle_right = min(msg.ranges[n//2 - 120 : n//2 - 80]) < 1.0
        return
