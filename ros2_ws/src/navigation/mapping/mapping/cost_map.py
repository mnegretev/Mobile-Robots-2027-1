    ...
    def get_cost_map(self, static_map, cost_radius):
        ...
        for i in range(height):
            for j in range(width):
                if static_map[i, j] == 100:
                    if static_map[i+1, j] == 100 and static_map[i-1, j] == 100 and \
                       static_map[i, j+1] == 100 and static_map[i, j-1] == 100:
                        continue
                    for k1 in range(-cost_radius, cost_radius + 1):
                        for k2 in range(-cost_radius, cost_radius + 1):
                            if (i+k1) < 0 or (i+k1) >= height or (j+k2) < 0 or (j+k2) >= width or \
                               static_map[i+k1, j+k2] == 100:
                                continue
