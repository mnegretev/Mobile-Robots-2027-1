...
public:
    ...
    std::vector<Eigen::Vector2d> recalculate_centroids(std::vector<Eigen::Vector2d>& centroids,
        ...
        // Inicializamos arreglos en 0 usando Eigen::Vector2d(0.0, 0.0)
        std::vector<Eigen::Vector2d> new_centroids(centroids.size(), Eigen::Vector2d(0.0, 0.0));       
        ...
        for (const auto& p : points) {
            ...
            for (size_t c = 0; c < centroids.size(); ++c) {
                // Eigen usa [0] para X y [1] para Y
                double dist = std::hypot(p[0] - centroids[c][0], p[1] - centroids[c][1]);
                if (dist < min_dist) {
                    min_dist = dist;
                    best_idx = c;
                }
            ...
            new_centroids[best_idx][0] += p[0];
            new_centroids[best_idx][1] += p[1];
            counters[best_idx] += 1;
        }
        ...
        for (size_t i = 0; i < centroids.size(); ++i) {
            if (counters[i] > 0) {
                new_centroids[i][0] /= counters[i];
                new_centroids[i][1] /= counters[i];
            } else {
                new_centroids[i] = centroids[i]; // Si no tuvo puntos, se queda donde estaba
            }
        }
        /*
         * END OF TODO
    ...
    void spin()
    {
        ...
        while(rclcpp::ok() && max_dist > this->tol){
            ...
            // Obtener la distancia máxima que se movió algún centroide
            max_dist = 0.0;
            for (size_t i = 0; i < centroids.size(); ++i) {
                double dist = std::hypot(centroids[i][0] - new_centroids[i][0], centroids[i][1] - new_centroids[i][1]);
                if (dist > max_dist) {
                    max_dist = dist;
                }
            }
            
            // Actualizar la lista de centroides viejos
