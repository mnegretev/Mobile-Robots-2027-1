...
#define FULL_NAME "Raymundo Romero Soto"
...
public:
    ...
    for (const auto& p : points) {
        ...
        for (size_t i = 0; i < centroids.size(); ++i) {
            double dist = (p - centroids[i]).norm(); // Norma en Eigen (distancia euclidiana)
            if (dist < min_dist) {
                min_dist = dist;
                idx = i;
            }
    ...
    for (size_t i = 0; i < centroids.size(); ++i) {
        if (counters[i] > 0) {
            ...
            new_centroids[i] = centroids[i]; // Mantener posición si no tuvo puntos asignados
        }
    }
    /*
     * END OF TODO
    ...
    auto start_time = std::chrono::high_resolution_clock::now(); // REGISTRO TIEMPO INICIAL
    ...
    while(rclcpp::ok() && max_dist > this->tol){
        ...
        max_dist = 0.0;
        for (size_t i = 0; i < centroids.size(); ++i) {
            double dist = (centroids[i] - new_centroids[i]).norm(); // Resta de vectores y norma Eigen
            if (dist > max_dist) {
                max_dist = dist;
            }
        }
    ...
    auto end_time = std::chrono::high_resolution_clock::now(); // REGISTRO TIEMPO FINAL
    std::chrono::duration<double> elapsed = end_time - start_time;

    RCLCPP_INFO(this->get_logger(), "Converged K centroids after %d iterations in %.4f seconds", 
