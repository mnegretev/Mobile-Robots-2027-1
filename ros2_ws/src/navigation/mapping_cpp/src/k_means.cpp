...
#define FULL_NAME "German Segovia Merlin--2027-1"
...
public:
    ...
    std::vector<Eigen::Vector2d> recalculate_centroids(std::vector<Eigen::Vector2d>& centroids,
        ...
        for(size_t i = 0; i < points.size(); i++){
            ...
            for(size_t j = 0; j < centroids.size(); j++){
                double dist = (points[i] - centroids[j]).norm();
                if(dist < min_dist){
                    min_dist = dist;
                    closest = j;
                }
        ...
        // Calcular el promedio y si el cluster esta vacio se deja el anterior
        for(size_t i = 0; i < centroids.size(); i++){
            if(counters[i] > 0){
                new_centroids[i] /= static_cast<double>(counters[i]);
            }
            else{
                new_centroids[i] = centroids[i];   // mantener el centroide anterior si no tiene puntos
            }
        }
    ...
    void spin()
    {
        ...
        while(rclcpp::ok() && max_dist > this->tol){
            ...
            // Calcular la distancia maxima entre los centroides viejos y los nuevos
            max_dist = 0.0;
            for(size_t i = 0; i < centroids.size(); i++){
