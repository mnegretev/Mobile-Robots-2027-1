...
#include <cstdio>
#include <limits>
#include <chrono> // Incluido para medir el tiempo
#include <Eigen/Dense>
...
public:
    ...
    double total_time_ms = 0.0; // Variable para acumular el tiempo total de cálculo

    while(rclcpp::ok() && max_dist > this->tol){
        pub_marker->publish(mrk);
        rclcpp::spin_some(this->get_node_base_interface());
        rclcpp::sleep_for(std::chrono::milliseconds(100)); // Esto añade latencia que no queremos medir
        
        // Iniciamos la medición del tiempo justo antes de hacer la matemática
        ...
        // Detenemos el temporizador y sumamos el tiempo de esta iteración
        auto end_time = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> iter_duration = end_time - start_time;
        total_time_ms += iter_duration.count();

        RCLCPP_INFO(this->get_logger(), "Max change in centroids: %lf", max_dist);
        mrk = get_centroids_marker(centroids);
    }

    // Imprimimos el tiempo final al terminar el ciclo (el que irá a tu tabla comparativa)
    RCLCPP_INFO(this->get_logger(), "TIEMPO TOTAL DEL ALGORITMO (sin ROS sleeps): %f ms", total_time_ms);
